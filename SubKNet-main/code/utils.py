...
def comput_scores(y_test, y_pred, th=0.5):
    ...
    return [aupr, auc, f1, acc, sen, spe, pre,MCC]
...
def comp_feat(MD, dataset):
    ...
    Ms3 = one_hot(labels_onehot_M, MD.shape[0] + MD.shape[1]).toarray()
    Ds3 = one_hot(labels_onehot_D, MD.shape[0] + MD.shape[1]).toarray()
