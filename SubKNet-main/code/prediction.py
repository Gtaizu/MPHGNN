...
def preds(MDs,adj,features,nodes,y,m_feat,d_feat,bsize,epoch,model,optimizer=None,criterion=None):
    ...
    pos=0
    outputs1 = np.zeros((N, 2))  # 正确初始化
    targets = np.zeros(N, dtype=int)  # 存储真实标签
    for i in range(0, N, bsize):
        ...
        if len(x_sub_adj)==1:
           continue
        outputs,loss2,outputs_srl,outputs_grl = model(torch.FloatTensor(MDS).cuda(),x_sub_adj.cuda(), x_sub_feat.cuda(), 
        ...
        # 按原始索引存储
        for node in range(n_graphs):
            original_idx = index[i + node]
            outputs1[original_idx] = output1[node]  # 直接赋值 NumPy 数组
            targets[original_idx] = y_batch[node]
    cvs=comput_metrics(outputs1,targets)
    return loss,outputs1,targets,cvs